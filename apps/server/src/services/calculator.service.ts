...
export class CalculatorService {
    ...
    async addHistoricRecordAsync(): Promise<void> {
        const accounts = await this.dataService.getAccounts();
        const historicRecords = this.accounting.buildHistoricRecords(accounts);

        await Promise.all(historicRecords.map(record => this.dataService.upsertHistoricRecord(record)));
    }
}